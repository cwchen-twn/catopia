...
- `src/components/nav.tsx` includes a `/blog` link; `src/app/sitemap.ts` adds `/blog` (via the existing `APP_ROUTES` auto-discovery — no extra code needed since `blog/page.tsx` is a top-level `[locale]` directory) plus each post's `/blog/<slug>` via `getBlogPosts()`.
