...
- Per-locale SEO metadata (title, description, keywords, OG locale, hreflang) lives in `src/app/[locale]/layout.tsx` via `generateMetadata`. `openGraph.images`/`twitter.images` point at `og-share-${locale}.png` (see "Open Graph Images" below).
...
**Open Graph images** — `public/images/og-share-en.png` / `-es.png` / `-pt.png`, one per locale, each with its own translated tagline. Generated by `scripts/generate-og-image.ts` (`bun run og-image`), which renders an SVG (logo + "Catopia" + tagline) per locale and rasterizes it with `sharp`. The locale/tagline map is hardcoded in the script (not read from `messages/*.json`) since this is a build tool, not app i18n — keep the taglines in sync with `home.hero.title` by hand if that copy changes. Regenerate and commit the PNGs (checked into git, not built in CI) whenever the tagline or logo changes; `src/app/[locale]/layout.tsx`'s `generateMetadata` references `og-share-${locale}.png` directly, no env baking needed since these are just static asset paths.

### Analytics

**Umami** (self-hosted at `umami.chenantunez.com`) is wired up in `src/app/[locale]/layout.tsx` — a `next/script` tag (`strategy="afterInteractive"`) with `data-website-id={process.env.NEXT_PUBLIC_UMAMI_WEBSITE_ID}`, gated behind `process.env.NODE_ENV === "production"` so `bun dev` sessions never get tracked (same pattern as the Turnstile production-only gate). `NEXT_PUBLIC_UMAMI_WEBSITE_ID` follows the established public-value baking pattern: read from `.dev.vars` via `readPublicVar()` in `next.config.ts`, pushed to GitHub Actions secrets by `scripts/set-gh-secrets.sh`, passed as a build-time `env:` in `deploy.yml`'s "Build and deploy" step — **not** a Cloudflare Worker runtime secret, so it's intentionally excluded from `scripts/set-cf-secrets.sh`.

Google Search Console and Microsoft Clarity remain deferred — no code work planned for those yet (see issue #3).

### Blog

Markdown-file-based, statically rendered at build time — ships with zero posts and needs no code changes to add one.

- **`content/blog/<slug>/<locale>.md`** — one file per locale per post, grouped by slug so translations stay aligned and the URL slug is identical across locales (good for hreflang). Frontmatter: `title`, `description`, `date`, optional `published` (defaults `true`). A locale file is optional per post — omitting one just means that translation doesn't exist yet, not an error. See `content/blog/README.md` for the exact format.
- **`bun run new-post <slug>`** (`scripts/new-blog-post.ts`) scaffolds all three locale files at once — title-cased from the slug, today's date, placeholder description/body per locale, and `published: false` by default so a scaffolded post never goes live by accident. Guards against a missing/invalid slug (lowercase letters, numbers, hyphens only) and an existing `content/blog/<slug>/` directory.
- **`next.config.ts`**'s `discoverBlogPosts()` scans `content/blog/*/` at build time, parses each locale file's frontmatter with `gray-matter` and renders its body to HTML with `marked`, sorts per locale by date descending, and bakes the result into `nextConfig.env.BLOG_POSTS` — same "filesystem work happens in `next.config.ts`, not in a prerendered page" pattern as `discoverRoutes()`/`APP_ROUTES` (see "Build-Time Filesystem Access" below).
- **Drafts** — a post with `published: false` is excluded when `process.env.NODE_ENV === "production"` inside `discoverBlogPosts()`. Next's CLI sets `NODE_ENV` before `next.config.ts` loads (`development` for `next dev`, `production` for `next build`), so this is a real runtime check in that one file, not a compile-time inlined literal like the client/server `TURNSTILE_ENABLED` checks — but it produces the same effect: `bun dev` shows drafts at `/blog/<slug>` for local preview, while `bun preview`/`bun run deploy` (both run `next build`) never bake them into the deployed bundle at all.
- **`src/lib/blog.ts`** — `getBlogPosts(locale)` / `getBlogPost(locale, slug)`, both just parsing `process.env.BLOG_POSTS` once. Shared by the blog routes and `sitemap.ts`.
- **`src/app/[locale]/blog/page.tsx`** — force-static index; shows `blog.emptyState` when the locale has zero posts.
- **`src/app/[locale]/blog/[slug]/page.tsx`** — force-static; `generateStaticParams` from `getBlogPosts(locale)`; renders the pre-rendered HTML body via `dangerouslySetInnerHTML` (safe here — it's compiled from author-controlled markdown at build time, not user input); `notFound()` if the slug/locale combo doesn't exist. `generateMetadata` uses the post's `title`/`description`.
- **`.blog-content` in `globals.css`** — minimal hand-written typography (headings, paragraphs, lists, links, inline code) for the rendered markdown body, since Tailwind's preflight strips default element styles and this project doesn't have `@tailwindcss/typography` installed.
- `src/components/nav.tsx` shows the `/blog` link only when `hasBlogPosts` is `true` — computed server-side in `[locale]/layout.tsx` as `getBlogPosts(locale).length > 0` and passed down as a prop, rather than importing `getBlogPosts` directly into `nav.tsx` (a client component), which would ship every post's full compiled HTML body to the client bundle just to check a count. This is per-locale: a locale with zero published posts hides the link even if another locale has some. `src/app/sitemap.ts` adds `/blog` (via the existing `APP_ROUTES` auto-discovery — no extra code needed since `blog/page.tsx` is a top-level `[locale]` directory) plus each post's `/blog/<slug>` via `getBlogPosts()`.
