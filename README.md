...
**CI/CD** — `.github/workflows/deploy.yml` runs on every `v*` tag push: applies pending D1 migrations (`bun run d1-migrate`), builds and deploys (`bun run deploy`, with `NEXT_PUBLIC_TURNSTILE_SITE_KEY` and `NEXT_PUBLIC_UMAMI_WEBSITE_ID` passed as build-time `env:`), then runs `bun run set-cf-secrets` with `RESEND_*`, `TURNSTILE_SECRET_KEY`, `CLOUDFLARE_API_TOKEN`, and `CLOUDFLARE_ACCOUNT_ID` all sourced from GitHub Actions repository secrets and passed via `env:`.
...
[Umami](https://umami.is/) (self-hosted at `umami.chenantunez.com`), wired up in `src/app/[locale]/layout.tsx` via `next/script`. Loads only in production builds (`process.env.NODE_ENV === "production"`) — `bun dev` sessions are never tracked.

`NEXT_PUBLIC_UMAMI_WEBSITE_ID` follows the same public-value pattern as `NEXT_PUBLIC_TURNSTILE_SITE_KEY`: set it in `.dev.vars`, baked into the client bundle at build time via `next.config.ts`, pushed to GitHub Actions secrets by `bun run set-gh-secrets`, passed as a build-time `env:` in `deploy.yml`. It's a public value, not a Worker runtime secret, so `set-cf-secrets` intentionally skips it.

Google Search Console and Microsoft Clarity are deferred — no code work planned for those yet.
